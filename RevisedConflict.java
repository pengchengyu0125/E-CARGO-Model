...
//    private List<List<Integer>> conflitLists;
    ...
    public void GRACAR_ILOG(int nagent, int nrole, double[][] QM, int[][] AC, int[] LIN, int[] LAIN, int[] DQ, int[] SC, double[] B, double[] P, int m1, int n1, double[] TV, double[] DS, int n2, int nc, double[][] CCE) {
        m = nagent;
        n = nrole;
        this.CCE = CCE;
        this.nc = nc;
...
//        this.conflitLists = conflitLists;
    ...
    public double solution(int[][] TR, List<List<Integer>> serverProviders) {
        try {
            //Creat cplex obj
            IloCplex cplex = new IloCplex();    //initialize the cplex object
...
//					if ((j+1)%(n) == 0) {System.out.print("\n");}
    ...
    public double test(int[][] TR, List<List<Integer>> serverProviders) {
        try {
            //Creat cplex obj
            IloCplex cplex = new IloCplex();    //initialize the cplex object
...
//					if ((j+1)%(n) == 0) {System.out.print("\n");}
    ...
    public static void main(String[] args) {
        ...
        for (int times = 0; times < excute; times++) {
            ...
            int m1 = m / 10 * 7, n1 = (int) (n * 0.9);
            //low sensitivity fields
            int n2 = (int) (n * 0.1 * 0.75);
            //role vector
            int[] L = new int[n];
            //data field quantity
            int[] DQ = new int[n];
            ...
            for (int i = 0; i < n; ++i) {
                ...
                L[i] = 1;
                //set data quantity
                DQ[i] = (generator.nextInt(3) + 1) * 500;
                //set budget
//                B[i] = generator.nextInt(4) * 100 + 300;
            ...
            for (int i = 0; i < m; ++i) {
                ...
                SC[i] = (int) (Math.pow(2, generator.nextInt(11)) * 16);
                //set LA
                if (i < m1) {
                    LA[i] = 10;
                } else {
...
//            printIMatrix(AC, m, m);
            ...
            //TEST parameters:
            int[][] T = new int[m][n];
            int nc = (n - n1) * (n - n1 - 1) * numbers * (numbers - 1) * sizeOfSP;
            double[][] CCE = new double[nc][5];
            double[][] CF = new double[n2][n - n1];
            for (int i = 0; i < n2; i++) {
                for (int j = i + 1; j < n - n1; j++) {
                    if (j < n2) {
                        CF[i][j] = (double) (generator.nextInt(5) + 1) / -10;
                        CF[j][i] = CF[i][j];
                    }
                    else {
                        CF[i][j] = (double) (generator.nextInt(5) + 6) / -10;
                    }
                }
            }
            int num = 0;
            for (int i = 0; i < serverProviders.size(); i++) {
                for (int j = 0; j < serverProviders.get(i).size(); j++) {
                    int x = serverProviders.get(i).get(j);
                    for (int k = 0; k < serverProviders.get(i).size(); k++) {
                        int y = serverProviders.get(i).get(k);
                        if (x != y) {
                            for (int l = n1; l < n; l++) {
                                for (int o = n1; o < n; o++) {
                                    if (l != o) {
                                        ...
                                        CCE[num][3] = o;
                                        if (l < n1 + n2) {
                                            CCE[num][4] = CF[l - n1][o - n1];
                                        }
                                        else if (o < n1 + n2) {
                                            CCE[num][4] = CF[o - n1][l - n1];
                                        }
                                        else {
                                            CCE[num][4] = 0;
                                        }
            ...
            RevisedConflict ILOG = new RevisedConflict();
            ILOG.GRACAR_ILOG(m, n, Q, AC, L, LA, DQ, SC, B, P, m1, n1, TV, DS, n2, nc, CCE);
...
//                }
//            }
            double result = ILOG.solution(T, serverProviders);
            //判断冲突
...
//            }
//            for (int i = 0; i < nc; i++) {
//                int[] assign1 = new int[2];
//                int[] assign2 = new int[2];
//                assign1[0] = (int) CCE[i][0];
//                assign1[1] = (int) CCE[i][1];
//                assign2[0] = (int) CCE[i][2];
//                assign2[1] = (int) CCE[i][3];
//                boolean flag1 = false;
//                boolean flag2 = false;
//                for (int j = 0; j < list.size(); j++) {
//                    int[] serv = list.get(j);
//                    if (serv[0] == assign1[0] && serv[1] == assign1[1]) {
//                        flag1 = true;
//                    }
//                    if (serv[0] == assign2[0] && serv[1] == assign2[1]) {
//                        flag2 = true;
//                    }
//                    if (flag1 && flag2) {
//                        result1 += CCE[i][4] * (Q[assign1[0]][assign1[1]] + Q[assign2[0]][assign2[1]]);
//                        break;
//                    }
//                }
//            }
